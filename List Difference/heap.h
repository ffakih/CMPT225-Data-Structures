...
public:
   ...
   // Modifiers
   void insertH( int element, int priority ); // Insert the pair <element,priority>.
...
Heap::Heap( const int * Priorities, const int * Elements, int s, int c){ 
   ...
   A=new Pair[hCapacity];
   hSize=0;
   for(int i=0; i<s;i++){insertH(Elements[i], Priorities[i]);}	
}
...
Heap::Heap( const Heap & Heap1, const Heap & Heap2, int c ){ 
   ...
   for (int i=0; i<Heap1.hSize; i++) {
      insertH (Heap1.A[i].element, Heap1.A[i].priority);
   }

   for (int j=0; j<Heap2.hSize; j++) {
      insertH (Heap2.A[j].element, Heap2.A[j].priority);
   }
...
void Heap::insertH(int element, int priority){
...
void Heap::trickleUp(int i){
   ...
   if (i != 0 && A[i].priority > A[parent].priority) {
      swap (i, parent);
      trickleUp(parent);              // Changed Signs to find Max element for extractMin 
   }
}
...
void Heap::trickleDown(int i){
   ...
   if (2*i+1 < hSize) {
      ...
      if (2*i+2 > hSize && A[2*i+2].priority > A[2*i+1].priority) {          //CHanged signs to find max element for extractMin
         temp = 2*i+2;
      }
